...
def find_matching_indices(values, target, max_combinations, used_indices):
    for r in range(1, max_combinations + 1):
        for indices in combinations(range(len(values)), r):
            if all(idx not in used_indices for idx in indices):  # Check if indices have been used
...
def match_transactions(df_groupA, df_groupB, max_combinations=4):
    matches = {}
    used_indices_b = set()  # Track used indices from group B
    ...
    for idxA, rowA in df_groupA.iterrows():
        amountA = rowA['$']
        matching_indices = find_matching_indices(df_groupB_sorted['$'].tolist(), amountA, max_combinations, used_indices_b)

        # If matches are found, store them and mark as used
        if matching_indices:
            matches[rowA['#']] = df_groupB_sorted.iloc[matching_indices]['#'].tolist()
            used_indices_b.update(matching_indices)  # Add the indices to the set of used indices
